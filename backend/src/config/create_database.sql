...
DELIMITER / / CREATE TRIGGER check_employee_age BEFORE INSERT ON Employee FOR EACH ROW BEGIN IF DATEDIFF (CURDATE (), NEW.birthDate) / 365.25 < 18 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Nhân viên phải từ 18 tuổi trở lên.';
...
DELIMITER / / CREATE TRIGGER CHK_dob BEFORE INSERT ON Customer FOR EACH ROW BEGIN IF NEW.dob > CURRENT_DATE() THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Ngày sinh không thể lớn hơn ngày hiện tại.';

END IF;

IF YEAR (CURRENT_DATE()) - YEAR (NEW.dob) < 16 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Khách hàng phải từ đủ 16 tuổi trở lên.';
...
DELIMITER / / CREATE TRIGGER CHK_served_date BEFORE INSERT ON ServedDate FOR EACH ROW BEGIN IF NEW.dateOfServing IS NULL THEN
SET
    NEW.dateOfServing = CURRENT_DATE;

END IF;

IF NEW.dateOfServing > CURRENT_DATE() THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Ngày phục vụ không thể lớn hơn ngày hiện tại.';
...
DELIMITER / / CREATE TRIGGER CHK_account_open_date BEFORE INSERT ON Account FOR EACH ROW BEGIN IF NEW.openDate IS NULL THEN
SET
    NEW.openDate = CURRENT_DATE;

END IF;

IF NEW.openDate > CURRENT_DATE() THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Ngày tạo tài khoản không thể lớn hơn ngày hiện tại.';
...
DELIMITER / / CREATE TRIGGER CHK_account_loan_taken_date BEFORE INSERT ON LoanAccount FOR EACH ROW BEGIN IF NEW.dateOfTaken IS NULL THEN
SET
    NEW.dateOfTaken = CURRENT_DATE();

END IF;

IF NEW.dateOfTaken > CURRENT_DATE() THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Ngày nhận khoản vay không thể lớn hơn ngày hiện tại.';
...
ALTER TABLE Branch ADD CONSTRAINT FK_Branch_Employee FOREIGN KEY (employeeID) REFERENCES Employee (employeeID);